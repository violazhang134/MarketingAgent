...
│   ├── studio/                   # 游戏生图工作室
│   │   ├── [toolId]/             # 工具工作区: 11 种创作工具
│   │   └── kit/                  # UI 套件生成向导 [NEW]
│   │       ├── page.tsx          # Phase 1: 风格 + 品类选择
│   │       └── generate/         # Phase 2: 批量生成
│   ├── create/                   # 素材创作流程: 创意工厂
│   ├── feed/                     # 决策流: Next Best Action
│   ├── launch/                   # Launch 流程: 报告 & 发布包
│   └── page.tsx                  # 首页: 中央控制塔
│
├── components/                   # UI 组件层
│   ├── canvas/                   # 无限画布模块
...
└── lib/                          # 核心逻辑与状态层
    ├── data/                     # 静态数据模板 [NEW]
    │   ├── style-templates.ts    # 8 种风格模板
    │   └── genre-templates.ts    # 5 种品类模板
    ├── engines/                  # 业务计算引擎
    │   ├── analysis-engine.ts
    │   ├── creative-engine.ts
    │   └── mock-data.ts
...
| 2024-12-31 | **游戏生图工作室** | 新增 `/studio` 路由，模型广场 + 历史记录 MVP                         |
| 2025-01-01 | **营销生图集成**   | 集成 `/api/generate-image` 到 Marketing 流程，支持封面/Banner/社交图 |
